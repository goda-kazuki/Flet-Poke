    ...
    def get_pokemon_by_type(type_name):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                # ポケモン一覧を表示
                if data["pokemon"]:
                    api_result.value = f"Found {len(data['pokemon'])} Pokemon of type '{type_name.capitalize()}'"

                    # 最初の20匹まで表示（並列処理で高速化）
                    pokemon_names = [
                        info["pokemon"]["name"] for info in data["pokemon"][:40]
                    ]
                    ...
                    # 取得した詳細情報を使ってカードを作成
                    for i, details in enumerate(pokemon_details_list):
                        pokemon_name = pokemon_names[i]

                        if details:
                            pokemon_card = ft.Card(
                                content=ft.Container(
                                    content=ft.Column(
                                        [
                                            ft.Image(
                                    ...
                                    ink=True,
                                    on_click=lambda e,
                                    pokemon_details=details: show_pokemon_detail(
                                        pokemon_details
                                    ),
                                )
                            )
